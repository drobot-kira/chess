...
public class Game {
    private byte[][] getField(int GameId) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(
                "C:\\Users\\Lenovo\\Desktop\\server\\Vadym\\src\\main\\java\\ua\\kpi\\chess\\field.txt"))) {
            String line;
    ...
    private void writeMove(int GameId, byte[][] field) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("field.txt"))) {
            for (byte[] row : field) {
                for (byte num : row) {
                    writer.write(num + " ");  // Записуємо числа через пробіл
                }
    ...
    public byte[][] SquareClicked(int GameId, byte SquareId, int UserId) {
        ...
        if ((field[SquareId / 10][SquareId % 10] == 1 && WhiteId == UserId) ||
                (field[SquareId / 10][SquareId % 10] == 2 && WhiteId != UserId)) // user's piece
        {
        ...
        } else // not user's piece
        {
            ...
            if (selectedPiece != -1) // there is a selected piece
            {
                if (field[SquareId / 10][SquareId % 10] < 0) // the move is possible
                {
                    ...
                    byte theEnd = Position.IsGameEnded(field);
